...
namespace TestLibrary.Instruments {
    public static class INSTRUMENT {
        ...
        public const String CHANNEL_1_2 = "(@1:2)";
    ...
    public class Instrument {
        ...
        private static readonly Dictionary<INSTRUMENT.ID, String> _instrumentAddresses = new Dictionary<INSTRUMENT.ID, String> {
            { INSTRUMENT.ID.Keysight_33461A_Multi_Meter, "USB0::0x2A8D::0x1301::MY60049978::0::INSTR" },
         // { INSTRUMENT.ID.Keysight_E33509B_WaveForm_Generator, "USB0::0x0957::0x2507::MY59003604::0::INSTR" },
            { INSTRUMENT.ID.Keysight_E36103B_Power_Supply_1, "USB0::0x2A8D::0x1602::MY61001983::0::INSTR" },
            { INSTRUMENT.ID.Keysight_E36103B_Power_Supply_2, "USB0::0x2A8D::0x1602::MY61001958::0::INSTR" },
         // { INSTRUMENT.ID.Keysight_E36105B_Power_Supply, "USB0::0x2A8D::0x1802::MY61001696::0::INSTR" },
            { INSTRUMENT.ID.Keysight_E36234A_Power_Supply, "USB0::0x2A8D::0x3402::MY61002598::0::INSTR" },
            { INSTRUMENT.ID.Keysight_EL34143A_Electronic_Load, "USB0::0x2A8D::0x3802::MY61001295::0::INSTR" }
            };
        public INSTRUMENT.ID ID { get; private set; }
        public INSTRUMENT.CATEGORY Category { get; private set; }
        public String Address { get; private set; }
        public object Instance { get; private set; }
        public String Manufacturer { get; private set; }
        public String Model { get; private set; }

        private Instrument(INSTRUMENT.ID id, String address) {
            this.ID = id;
            this.Address = address;

            try {
                switch (ID) {
                    case INSTRUMENT.ID.Keysight_E36103B_Power_Supply_1:
                    case INSTRUMENT.ID.Keysight_E36103B_Power_Supply_2:
                    case INSTRUMENT.ID.Keysight_E36105B_Power_Supply:
                        this.Category = INSTRUMENT.CATEGORY.Power_Suppy;
                        this.Instance = new AgE3610XB(this.Address);
                        ((AgE3610XB)this.Instance).SCPI.SYSTem.RWLock.Command();
                        break;
                    case INSTRUMENT.ID.Keysight_E36234A_Power_Supply:
                        this.Category = INSTRUMENT.CATEGORY.Power_Suppy;
                        this.Instance = new AgE36200(this.Address);
                        ((AgE36200)this.Instance).SCPI.SYSTem.RWLock.Command();
                        break;
                    case INSTRUMENT.ID.Keysight_EL34143A_Electronic_Load:
                        this.Category = INSTRUMENT.CATEGORY.Electronic_Load;
                        this.Instance = new AgEL30000(this.Address);
                        ((AgEL30000)this.Instance).SCPI.SYSTem.RWLock.Command();
                        break;
                    case INSTRUMENT.ID.Keysight_E33509B_WaveForm_Generator:
                        this.Category = INSTRUMENT.CATEGORY.Waveform_Generator;
                        this.Instance = new Ag33500B_33600A(this.Address);
                        break;
                    case INSTRUMENT.ID.Keysight_33461A_Multi_Meter:
                        this.Category = INSTRUMENT.CATEGORY.Multi_Meter;
                ...
                throw new InvalidOperationException($"Check to see if {Enum.GetName(typeof(INSTRUMENT.CATEGORY), this.Category)} with VISA Address '{this.Address}' is powered and it's {a[0]} bus is communicating.", e);
            }
        }

        public static Dictionary<INSTRUMENT.ID, Instrument> Get() {
            Dictionary<INSTRUMENT.ID, Instrument> d = new Dictionary<INSTRUMENT.ID, Instrument>();
            foreach (KeyValuePair<INSTRUMENT.ID, String> ia in _instrumentAddresses) d.Add(ia.Key, new Instrument(ia.Key, ia.Value));
            return d;
        }
    }
    ...
    public static class InstrumentTasks {
        ...
        public static void SCPI99_Reset(Dictionary<INSTRUMENT.ID, Instrument> instruments) {
            foreach (KeyValuePair<INSTRUMENT.ID, Instrument> i in instruments) SCPI99.Reset(i.Value.Address);
        }

        public static void SCPI99_Clear(Dictionary<INSTRUMENT.ID, Instrument> instruments) {
            foreach (KeyValuePair<INSTRUMENT.ID, Instrument> i in instruments) SCPI99.Clear(i.Value.Address);
        }
    
        public static void SCPI99_ResetClear(Dictionary<INSTRUMENT.ID, Instrument> instruments) {
            foreach (KeyValuePair<INSTRUMENT.ID, Instrument> i in instruments) {
                SCPI99.Reset(i.Value.Address);
                SCPI99.Clear(i.Value.Address);
            }
        }

        public static void SCPI99_Test(Dictionary<INSTRUMENT.ID, Instrument> instruments) {
        ...
        public static void InstrumentResetClear(Dictionary<INSTRUMENT.ID, Instrument> instruments) {
            foreach (KeyValuePair<INSTRUMENT.ID, Instrument> i in instruments) {
                switch (i.Value.ID) {
                    case INSTRUMENT.ID.Keysight_E36103B_Power_Supply_1:
                    case INSTRUMENT.ID.Keysight_E36103B_Power_Supply_2:
                    ...
                    case INSTRUMENT.ID.Keysight_EL34143A_Electronic_Load:
                        ...
                        break;
                    case INSTRUMENT.ID.Keysight_E33509B_WaveForm_Generator:
                        // TODO: case INSTRUMENT.ID.Keysight_E33509B_WaveForm_Generator:
                        break;
                    case INSTRUMENT.ID.Keysight_33461A_Multi_Meter:
                        // TODO: case INSTRUMENT.ID.Keysight_33461A_Multi_Meter:
                        break;
                    default:
                        throw new NotImplementedException($"Unrecognized Instrument!{Environment.NewLine}{Environment.NewLine}" +
